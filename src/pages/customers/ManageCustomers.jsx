...
import {
  ...
  updateCustomer,
  uploadDocument,
} from "../../feature/customers/manageCustomerSlice"; // Adjust path as needed
...
export default function ManageCustomers() {
  ...
  // -- REDUX STATE --
  const { leads, isLoading, isSaving, error } = useSelector(
  ...
  // Non-serializable Blob UI state stays local
  const [docBlobUrls, setDocBlobUrls] = useState({});
  const [isLoadingDocs, setIsLoadingDocs] = useState(false);
  const [replacingDocId, setReplacingDocId] = useState(null);

  const [isStatusModalOpen, setIsStatusModalOpen] = useState(false);
  const [statusUpdating, setStatusUpdating] = useState(false); // Local loader for status
  const [tempStatus, setTempStatus] = useState("");

  const getToken = () => localStorage.getItem("token");

  const { userInfo } = useSelector((state) => state.auth);
  const companyId = userInfo?.companyId; // Change to userInfo?.companyId if that's how your DB stores it
  const branchId = userInfo?.branchId; // Assuming you have this for your document upload
  ...
  const DOCUMENT_MAP = {
    "Applicant Photo": "docApplicantPhoto",
    "PAN Card": "docPanCard",
    "Aadhar Front": "docAadhaarFront",
    "Aadhar Back": "docAadhaarBack",
    "Residence Proof": "docCurrentAddress",
    "Permanent Address Proof": "docPermanentAddress",
    "Bank Statement": "docBankStatement",
    "Salary Slip": "docIncomeProof",
    "Cancelled Cheque": "docCancelledCheque",
    Passbook: "docBankStatement", // if needed
    Other: "docOther",
  };
  ...
  const handleStatusUpdate = () => {
    ...
    const payload = {
      ...selectedCustomer,
      applicationStatus: tempStatus,
      correction_feedback: selectedCustomer.correction_feedback,
      approved_loan_amount: selectedCustomer.approved_loan_amount,
    };

    toast
      .promise(dispatch(updateCustomer(payload)).unwrap(), {
        loading: "Updating status...",
        success: "Status & Feedback updated successfully!",
  ...
  const openStatusModal = (customer) => {
    console.log("ddddddd", customer);
    setSelectedCustomer({ ...customer }); // Clone to local draft
    setTempStatus(customer.applicationStatus);
    setIsStatusModalOpen(true);
  };

  // Fetch blobs for documents (Stays local because Blobs don't belong in Redux)
  ...
  const handleInputChange = (e) => {
    ...
    // Update local draft state directly
  ...
  const handleSaveChanges = () => {
    ...
    toast
      .promise(dispatch(updateCustomer(selectedCustomer)).unwrap(), {
  ...
  const handleDocumentAction = (docName, file, existingDbId = null) => {
    if (!file || !selectedCustomer?.id) return;

    const backendKey = DOCUMENT_MAP[docName];

    if (!backendKey) {
      toast.error(`No backend mapping for: ${docName}`);
      return;
    }
    ...
    dispatch(
      ...
      .then((result) => {
        ...
        setDocBlobUrls((prev) => {
          if (prev[docName]?.url) {
            URL.revokeObjectURL(prev[docName].url);
          }
          ...
          <tbody className="divide-y divide-slate-50">
            {isLoading ? (
                ...
                >
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xs">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-mono">
                          #{lead.system_uniquecust_id}
                        </p>
                      </div>
                    </div>
                  </td>
                  <td className="p-4 text-sm text-slate-600 uppercase font-mono">
                    {lead.applicant_pan}
                  </td>
                  ...
                  <td className="p-4 font-bold text-blue-700">
                    ₹{lead.requested_loan_amount || "0"}
                  </td>
                  <td className="p-4">
                    <span
                      className={`px-2 py-1 rounded text-[10px] font-black uppercase shadow-sm ${
                        lead.applicationStatus === "Approved"
                          ? "bg-green-100 text-green-700"
                          : lead.applicationStatus === "Rejected"
              ...
              <tr>
                <td
                  colSpan="6"
                  className="p-10 text-center text-slate-400 text-sm italic"
                >
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200 flex flex-col">
            <div className="p-6 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
              <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <ShieldCheck size={18} className="text-blue-600" /> Application
                Status Hub
              </h3>
              <button
            ...
            <div className="p-8 overflow-y-auto flex-1 bg-white">
              <div className="flex flex-col items-center mb-10 text-center">
                ...
                <p className="font-bold text-slate-800 text-3xl tracking-tight">
                  {selectedCustomer.firstName} {selectedCustomer.lastName}
                </p>
                <span className="mt-2 px-3 py-1 bg-slate-100 rounded-full font-mono text-[10px] text-slate-500 uppercase tracking-widest">
                  System ID: {selectedCustomer.system_uniquecust_id}
                </span>
              </div>
              ...
              <div className="bg-slate-50/50 p-10 rounded-3xl border border-slate-100 shadow-inner mb-10">
                <div className="relative flex items-center justify-between w-full max-w-3xl mx-auto">
                  ...
                  <div
                    ...
                    style={{
                      width: `${(STATUS_OPTIONS.indexOf(selectedCustomer.applicationStatus) / (STATUS_OPTIONS.length - 1)) * 100}%`,
                    }}
                  ></div>
                  {STATUS_OPTIONS.map((step, index) => {
                    const stepIndex = STATUS_OPTIONS.indexOf(
                      selectedCustomer.applicationStatus,
                    );
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`h-10 w-10 rounded-full flex items-center justify-center transition-all duration-500 border-4 shadow-sm ${
                            isActive
                        ...
                        <p
                          className={`absolute -bottom-8 whitespace-nowrap text-[10px] font-black uppercase tracking-tighter ${
                            stepIndex === index
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-6">
                  ...
                  {tempStatus === "Approved" && (
                    <div className="p-6 bg-emerald-50 rounded-2xl border-2 border-emerald-100 shadow-md animate-in slide-in-from-left-4">
                      <label className="text-[10px] font-black text-emerald-600 uppercase mb-3 block tracking-widest">
                ...
                <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 flex flex-col">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2 mb-3">
                    <History size={14} className="text-blue-500" /> Update
                    Feedback
                  </label>
                  <textarea
            ...
            <div className="p-6 border-t bg-slate-50/50 flex justify-end gap-3">
              <button
                onClick={() => setIsStatusModalOpen(false)}
                className="px-8 py-3 rounded-xl border bg-white font-bold text-[11px] uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-colors"
              >
                Discard
              </button>
              <button
                ...
                className="px-10 py-3 rounded-xl bg-slate-900 text-white font-bold text-[11px] uppercase shadow-lg flex items-center gap-2 hover:bg-blue-600 transition-all disabled:opacity-50"
              >
                {statusUpdating ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 font-sans">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[92vh] flex flex-col overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200">
            <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter">
                  {modalMode === "edit" ? "Edit Portfolio" : "Lead Details"}
                </h3>
                <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-0.5 rounded font-bold uppercase tracking-widest">
                  {selectedCustomer.system_uniquecust_id}
                </span>
              </div>
              <button
                onClick={closeModal}
              ...
              {activeTab === "applicant" && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-2">
                  ...
                  <FormField
                    ...
                    name="monthlyIncome"
                    value={selectedCustomer.monthlyIncome}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                  <FormField
                    ...
                    value={selectedCustomer.requested_loan_amount}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                    type="number"
                  />
                  ...
                  <FormField
                    label="Email"
                    name="emailAddress"
                    value={selectedCustomer.emailAddress}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                  <FormField
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        ...
                        value={selectedCustomer.applicant_currentAddress}
                        onChange={handleInputChange}
                        modalMode={modalMode}
                      />
                      <div className="grid grid-cols-3 gap-2">
                        <FormField
                          label="City"
                          name="applicant_city"
                          value={selectedCustomer.applicant_city}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                        <FormField
                          ...
                          value={selectedCustomer.applicant_state}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                        <FormField
                          ...
                          value={selectedCustomer.applicant_pinCode}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                      </div>
                  ...
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField
                        ...
                        value={selectedCustomer.applicant_permanentAddress}
                        onChange={handleInputChange}
                        modalMode={modalMode}
                      />
                      <div className="grid grid-cols-3 gap-2">
                        <FormField
                          ...
                          value={selectedCustomer.applicant_permCity}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                        <FormField
                          label="State"
                          name="applicant_permState"
                          value={selectedCustomer.applicant_permState}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                        <FormField
                          ...
                          value={selectedCustomer.applicant_permPinCode}
                          onChange={handleInputChange}
                          modalMode={modalMode}
                        />
                      </div>
              ...
              {activeTab === "nominee" && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-bottom-2">
                  <FormField
                    ...
                    value={selectedCustomer.nominee_fullname}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                  <FormField
                  ...
                  <FormField
                    label="Mobile"
                    name="nominee_mobile"
                    value={selectedCustomer.nominee_mobile}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                  <FormField
                    label="PAN Card"
                    name="nominee_pan"
                    value={selectedCustomer.nominee_pan}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                  <FormField
                    ...
                    value={selectedCustomer.nominee_aadhaar}
                    onChange={handleInputChange}
                    modalMode={modalMode}
                  />
                </div>
              ...
              {activeTab === "bank" && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-bottom-2">
                  <div className="space-y-4">
                    <h4 className="text-xs font-black text-slate-800 uppercase border-l-4 border-blue-600 pl-3">
                      Applicant Account
                    </h4>
                    <FormField
                      label="Bank Name"
                    ...
                    <FormField
                      label="IFSC Code"
                      name="ifscCode"
                      value={selectedCustomer.ifscCode}
                      onChange={handleInputChange}
                      modalMode={modalMode}
                    />
                  </div>
                  <div className="space-y-4">
                    <h4 className="text-xs font-black text-slate-800 uppercase border-l-4 border-emerald-600 pl-3">
                      Nominee Account
                    </h4>
                    <FormField
                      label="Bank Name"
                      name="nominee_bankName"
                      value={selectedCustomer.nominee_bankName}
                      onChange={handleInputChange}
                      modalMode={modalMode}
                    />
                    <FormField
                      ...
                      value={selectedCustomer.nominee_accountNumber}
                      onChange={handleInputChange}
                      modalMode={modalMode}
                    />
                    <FormField
                      ...
                      value={selectedCustomer.nominee_ifscCode}
                      onChange={handleInputChange}
                      modalMode={modalMode}
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 animate-in fade-in">
                  {REQUIRED_DOCUMENTS.map((docName) => {
                    ...
                    return (
                      <div
                        key={docName}
                        className="border border-slate-200 rounded-2xl overflow-hidden bg-white shadow-sm relative group"
                      >
                        ...
                        <div className="p-3 bg-white border-t border-slate-50">
                          <p className="text-[10px] font-black text-slate-700 uppercase truncate mb-3">
                            {docName}
                          </p>
                          <div className="flex gap-2">
                            {hasFile && (
                              <a
                                ...
                                target="_blank"
                                rel="noreferrer"
                                className="flex-1 text-center py-1.5 bg-slate-50 rounded-lg text-[10px] font-black uppercase flex items-center justify-center gap-1 shadow-sm"
                              >
                                <Eye size={12} /> View
                              </a>
                            )}
              ...
              {activeTab === "journey" && (
                <div className="space-y-12 animate-in fade-in">
                  <div className="bg-slate-50 p-10 rounded-3xl border border-slate-100 shadow-inner">
                    <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-10 text-center">
                      Journey Roadmap
                    </h4>
                    <div className="relative flex items-center justify-between w-full max-w-3xl mx-auto">
                      <div className="absolute left-0 top-1/2 -translate-y-1/2 w-full h-1 bg-slate-200 z-0"></div>
                      ...
                      {STATUS_OPTIONS.map((step, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <p className="absolute -bottom-10 whitespace-nowrap text-[10px] font-black uppercase tracking-widest text-slate-400">
                              {step}
                            </p>
                          </div>
                        );
                    ...
                    <div className="space-y-6">
                      <div className="p-6 bg-slate-50 rounded-2xl border border-slate-100 shadow-sm">
                        ...
                        <select
                          ...
                          value={selectedCustomer.applicationStatus || ""}
                        ...
                        <div className="p-6 bg-emerald-50 rounded-2xl border-2 border-emerald-100 shadow-md animate-in slide-in-from-left-4">
                          ...
                          <div className="relative">
                            ...
                            <input
                              ...
                              name="approved_loan_amount"
                              value={
                                selectedCustomer.approved_loan_amount || ""
                              }
                              onChange={handleInputChange}
                              disabled={modalMode !== "edit"}
                              className="w-full pl-12 pr-4 py-4 rounded-2xl font-black text-xl text-emerald-700 outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
                      ...
                      {modalMode === "edit" ? (
                        ...
                        <div className="p-6 bg-slate-50 border border-slate-100 rounded-3xl min-h-[180px] flex flex-col">
                          ...
                          <div className="mt-4 flex items-center gap-2 text-[9px] font-black text-slate-400 uppercase tracking-widest">
                            <AlertCircle size={12} /> Customer is viewing this
                            message
                          </div>
                        </div>
                      )}
            ...
            <div className="px-8 py-5 border-t bg-slate-50/50 flex justify-end gap-3">
              <button
                onClick={closeModal}
                className="px-6 py-2 rounded-xl border bg-white font-bold text-[11px] uppercase tracking-widest text-slate-600 hover:bg-slate-50 transition-colors"
              >
                Discard
              </button>
              {modalMode === "edit" && (
                <button
                  ...
                  disabled={isSaving}
                  className="px-8 py-2 rounded-xl bg-blue-600 text-white font-bold text-[11px] uppercase shadow-lg flex items-center gap-2 hover:bg-blue-700 transition-all"
                >
                  {isSaving ? (
                    <Loader2 size={16} className="animate-spin" />
                  ) : (
